    ...
    return (
      <div>
        <div {...getRootProps({className: 'dropzone'})} style={{minHeight: '200px', border: '1px solid #ebedf2', padding: '15px', display: 'flex', flexDirection: 'column'}}>
          ...
          <button type="button" onClick={open} className="btn btn-gradient-primary mt-auto">
            Open File Dialog
          </button>
        </div>
        <aside>
                    ...
                    { this.state.users.map((user, index) => {
                        return (
                            <div key={user.id} className="d-flex mb-2">
                                ...
                                {(index > 0) ? <button className="btn btn-gradient-danger btn-sm icon-btn ml-2" onClick={()=>this.deleteUser(index)}><i className="mdi mdi-delete"></i></button> : null }
                            </div>
                        )
                    })}
                </div>
                <button className="btn btn-gradient-info btn-sm icon-btn ml-2 mb-2" onClick={ this.addUserInput } ><i className="mdi mdi-plus"></i></button>
            </form>
        )
    }
}
